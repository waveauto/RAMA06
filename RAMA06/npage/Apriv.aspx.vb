...
#Region "EVEN"
    ...
    Private Sub gvpic_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvpic.RowCommand
        If mFunc.fCheckSession Then
            ...
            ElseIf e.CommandName = "aDel" Then
                ...
                sqldelimg = "SELECT renamefile FROM oss_imgfoundpriv WHERE id_imgoss_foundpriv ='" & nID & "'"
                ...
                Dim sqldelete As String = "DELETE FROM oss_imgfoundpriv WHERE id_imgoss_foundpriv='" & nID & "'"
    ...
    Private Sub gvData_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvData.RowCommand
        If e.CommandName = "aEdit" Then
            ...
            hdfIDNew.Value = ndutyID

            Response.Redirect("~/nPage/Epriv.aspx?value1=" + hdfIDNew.Value)
        ...
        ElseIf e.CommandName = "atopic" Then
            ...
            Dim url As String = "Vpriv.aspx?value1=" + hdfIDNew.Value
        ...
        ElseIf e.CommandName = "aDel" Then
            ...
            If mDB.fReadDataTable(cmds, nDt) Then
                If nDt.Rows.Count > 0 Then
                    ...
                    Dim strsql As String = "SELECT * FROM oss_imgfoundpriv WHERE rID = '" & nDt.Rows(0).Item("rID") & "'"
                    ...
                    If mDB.fReadDataTable(cmdd, nDt) Then
                        If nDt.Rows.Count > 0 Then
                            For i = 0 To nDt.Rows.Count - 1
                                ...
                                delpic(nDt.Rows(i).Item("id_imgoss_foundpriv"))
                            Next
                        End If
                    End If
            ...
            Dim sql As String = "DELETE FROM oos_foundpriv WHERE IDpriv='" & hdfrID.Value & "'"
            Dim cmd As New SqlCommand(sql)
    ...
    Private Sub gvData_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvData.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim id_new As Integer = Int32.Parse(gvData.DataKeys(e.Row.RowIndex).Values("IDpriv").ToString())
...
#Region "FUNCTION"
    ...
    Private Function fGetSearchStr() As String
        Dim nValue As String = ""
        If txtSearch.Text <> "" Then nValue &= " and (privHead like @searchTxt )"
        Return nValue
    End Function
    ...
    Private Sub BindGrid()
        ...
        Dim nDt As DataTable
        Dim strsql As String = "SELECT row_number() OVER (ORDER BY id_imgoss_foundpriv ASC) AS row,* FROM oss_imgfoundpriv  WHERE rID='" & hdfrID.Value & "'"
    ...
    Private Function fSaveData(ByRef pValue As String) As Boolean
        ...
        nSb.Append("insert into oos_foundpriv(rID,privHead,privdetail,datesend,statusdate,datestart,dateend,statusicon,frequency,codestaff) ")
        nSb.Append("values(@rID,@privHead,@privdetail,@datesend,@statusdate,@datestart,@dateend,@statusicon,@frequency,@codestaff)")
        Dim nComd As New SqlCommand(nSb.ToString)
        fSetParaValue(nComd)
    ...
    Private Sub delpic(ByVal id As Integer)
        ...
        Dim sqldelimg As String
        Dim namefile As String
        sqldelimg = "SELECT renamefile FROM oss_imgfoundpriv WHERE id_imgoss_foundpriv ='" & id & "'"
        ...
        Dim sqldelete As String = "DELETE FROM oss_imgfoundpriv WHERE id_imgoss_foundpriv='" & id & "'"
        Dim cmddel As New SqlCommand(sqldelete)
